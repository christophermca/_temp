...
class DirectorHelper extends AppHelper {
	...
	function albumBg($value, $id) {
		...
		if (empty($value)) {
			...
			$value = p($value, $id, 48, 37, 1, 70, 0, $x, $y);
		}
	...
	function getVidThumb($src, $preview, $id, $w = 84, $h = 63, $sq = 1, $q = 70, $sh = 1, $tail = false) {
		if (strpos($preview, ':') !== false) {
			list($src, $x, $y) = explode(':', $preview);
			$filename = p($src, $id, $w, $h, $sq, $q, $sh, $x, $y);
		} else if (isVideo($src)) {
			$pos = strrpos($src, '.');
	...
	function avatar($user_id, $w = 48, $h = 48, $anchor, $extra_styles = null) {
		...
		if (!empty($original)) {
			...
			$out .= p(basename($original[0]), "avatar-{$user_id}", $w, $h, 1, 75, 1, $x, $y, false);
		} else {
	...
	// Dialogue wrappers
	////
	function preDialogue($id, $show = false, $w = null, $classes = '') {
		if (!empty($classes)) {
			$classes = ' ' . $classes;
		}
		return '<div id="' . $id . '" class="dialogue-wrap' . $classes . '"' . ($show ? '' : 'style="display:none;"') . '><div class="dialogue"><div class="dialogue-content"' . ife(!is_null($w), ' style="width:' . $w . 'px"') . '><div class="dialogue-inner-wrap">';
	}
	...
	function recentList($imgs, $max, $w, $h, $album = false) {
		...
		foreach($imgs as $img):
			...
			if (isNotImg($img['Image']['src'])):
				...
				$o .= p($img['Image']['src'], $img['Image']['aid'], 250, 250, 0, 70, 1, $x, $y);
			endif;
			...
			if (isNotImg($img['Image']['src'])):
				$o .= $this->getVidThumb($img['Image']['src'], $img['Image']['lg_preview'], $img['Image']['aid'], 84, 63, 1, 70, 1, true);
			else:
			 	$o .= p($img['Image']['src'], $img['Image']['aid'], 84, 63, 1, 70, 1, $x, $y);
			endif;
		
			$o .= '</span>';
			$o .= $this->Html->image('loading_ei.gif', array('width' => $w, 'height' => $h, 'alt' => 'Image'));
			if ($img['Image']['is_video']):
			$o .= $this->Html->image('vid_overlay.gif', array('width' => 16, 'height' => 16, 'class' => 'video-overlay'));
			endif;
			$o .= '</span></li>';
